...
template<class T, auto N> inline constexpr auto entries = [] {
  ...
  for (auto i = 0u; i < N; ++i) {
    ...
    while (true) {
      r = random();
      if (std::find_if(a.begin(), a.end(), [&](const auto& p){ return p.first == r; }) == a.end()) {
        break;
      }
    }
    a[i] = {r, data[i]};
...
template<u32 Size, double Probability, u64 Seed, auto N = 100'000>
[[nodiscard]] auto string_to_enum(auto fn) {
  ...
  std::array<std::vector<std::string_view>, 2u> data{};
